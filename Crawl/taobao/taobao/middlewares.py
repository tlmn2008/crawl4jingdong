...
class TbDownloadmiddlewareRandomUseragent(object):
    def __init__(self):
        ...
        try:
            ...
            if page > 1:
                input = self.wait.until(
                    # EC.presence_of_element_located((By.XPATH, './/input[class="input J_Input"]')))  # 获取输入页面数框
                    EC.presence_of_element_located((By.XPATH, '//*[@id="mainsrp-pager"]/div/div/div/div[2]/input')))  # 获取输入页面数框

                # submit = self.wait.until(EC.element_to_be_clickable((By.XPATH, '//*[@id="mainsrp-pagermainsrp-pager"]/div/div/div/div[2]/span[3]')))  # 获取确定按钮
                submit = self.wait.until(EC.element_to_be_clickable((By.XPATH, '//span[class="btn J_Submit"]')))  # 获取确定按钮
                input.clear()
            ...
            # self.wait.until(EC.element_to_be_clickable((By.XPATH, '//input[class="input J_Input"]')))
            # self.wait.until(EC.element_to_be_clickable((By.XPATH, '//*[@id="mainsrp-pager"]/div/div/div/div[2]/input')))
            time.sleep(1)
